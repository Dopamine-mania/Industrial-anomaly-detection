        ...
        for cls_name in self.CLSNAMES:
            ...
            for phase in ['test']:
                ...
                for specie in species:
                    ...
                    mask_names = os.listdir(f'{cls_dir}/ground_truth/') if is_abnormal else None
                    img_names.sort()
                    mask_names.sort() if mask_names is not None else None
                    for idx, img_name in enumerate(img_names):
                        info_img = dict(
                            img_path=f'{cls_name}/{phase}/{specie}/{img_name}',
                            mask_path=f'{cls_name}/ground_truth/{mask_names[idx]}' if is_abnormal else '',
                            cls_name=cls_name,
                            specie_name=specie,
...
if __name__ == '__main__':
    runner = IsicSolver(root=f'{DATASETS_ROOT}/isic')
    runner.run()
