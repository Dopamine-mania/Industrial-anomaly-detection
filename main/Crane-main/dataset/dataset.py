...
class Dataset(data.Dataset):
    def __init__(self, roots, transform, target_transform, dataset_name, kwargs=None):
        ...
        # Academic/anti-leakage guard: training must never touch test data, and must use normal-only.
        if dataset_split == 'train' and getattr(kwargs, "train_good_only", True):
            self.data_all = [s for s in self.data_all if int(s.get("anomaly", 0)) == 0]
